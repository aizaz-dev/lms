    ...
    <?php 
        ...
        else if($_GET["action"] == 'view')
        {
            ...
            if($issue_book_id > 0)
            {
                ...
                $sql = "
                UPDATE lms_issue_book 
                SET return_date_time = '".$today_date."', book_issue_status = 'Return', book_status = 'Enable'
                WHERE issue_book_id = '$issue_book_id';
            ";
            ...
            $connect->query($query);
                    ...
                    ';
                    echo '
                    <h2>Book Return Successfully</h2>
    	...
    	<div class="card-header">
    		<div class="row">
    			<div class="col col-md-6">
    				<i class="fas fa-table me-1"></i> Return Book Management
                </div>
        	...
        	<table id="datatablesSimple">
        		...
        		if($statement->rowCount() > 0)
        		{
        			...
        			foreach($statement->fetchAll() as $row)
        			{
        				...
        				if($status == 'Issue')
        				{
        					$status = '<span><span class="badge bg-warning">Issue</span><a href="issue_book.php?action=view&code='.convert_data($row["issue_book_id"]).'" class="btn btn-info btn-sm btn1">Clike here to Return</a>
                            </span>';
        				}

        				if($status == 'Not Return')
        				{
        					$status = ' class="d-flex align-items-center"> <span class="badge bg-danger">Not Return</span>';
        				...
        				echo '
        				<tr>
        					<td data-label="Book isbn Number">'.$row["book_id"].'</td>
        					<td data-label="User unique id">'.$row["user_id"].'</td>
        					<td data-label="Issue Date">'.$row["issue_date_time"].'</td>
        					<td data-label="Return Date">'.$row["return_date_time"].'</td>
        					<td data-label="Action">'.$status.'</td>
